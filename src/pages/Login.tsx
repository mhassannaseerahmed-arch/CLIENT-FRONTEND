...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setError(null);
    if (!email.trim()) {
      return setError("Email is required.");  // ✅ works fine
    }
    if (!password) {
      return setError("Password is required.");  // ✅ works fine
    }
    if (password.length < 8) {
      return setError("Password must be at least 8 characters.");  // ✅ works fine
    }
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 font-inter">
      ...
      {loading && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50">
          <div className="bg-white px-6 py-4 rounded-xl shadow-lg text-gray-700 font-semibold">
            <div role="status">
